      ...
      <fieldset class="form-section">
        ...
        <div class="form-row">
          <div class="stat-group">
            <label>Lihat (Views)</label>
            <p>{{ form.views }}</p>
          </div>
          <div class="stat-group">
            <label>Terjual (Purchases)</label>
            <p>{{ form.purchases }}</p>
          </div>
          <div class="stat-group">
            <label>Rating</label>
            <p>⭐ {{ form.rating.toFixed(1) }} ({{ form.reviewCount }} review)</p>
          </div>
        </div>
      </fieldset>
...
onMounted(async () => {
  try {
    ...
    const { data: itemData } = await $fetch(`/api/admin/items/${itemId}`)

    // Parse specs back to text format
    const specsText = itemData.specs
      ? JSON.parse(itemData.specs)
          .map((s) => `${s.label}|${s.value}`)
          .join('\n')
      : ''

    // Parse gallery images back to text format
    const galleryImagesText = itemData.galleryImages
      ? JSON.parse(itemData.galleryImages).join('\n')
      : ''
...
async function submitForm() {
  ...
  const payload = {
    ...
    levelRequirement: form.value.levelRequirement,
    classRequirement: form.value.classRequirement,
